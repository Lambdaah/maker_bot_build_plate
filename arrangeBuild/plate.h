...
{
private:
	int **buildPlate;  //Our 2d array, this is how we're going to represent our buildplate visually, 0 meaning empty, any positive int represents a object
	int size; //User's input size, array is going to be of size, size x size, in other words our dimensions
	point center ; //The center of the build plate, will be used in our manhantten distance formula and to place our first object
	std::vector<T> objectList; //List of all the objects we need to place
	std::unordered_map<int, std::pair<point, T>> placementList; //A list of the placed objects and the location of their starting point (where we begin to draw a H x W bounding box


	//Functions:
	bool addToPlate(point&, T&,int); //Adds an object to the build Plate given the starting point, the object itself for it's height and width as well as the object's id (to draw on the matrix/plate)
	void rotatePlate(); //Rotate the entire plate 90 degrees counter clockwise
	...
	void populatePlacementList(); //Goes through the buildplate and logs/records the starting placement of all objects into our placementList table

	//Brute force function, checks every position to see if we can place an object, will return the left/top most position such that it's closest to the center
	void bruteForce(T&,int);

	//Bound finders, these simply find the first/last row/col of our current figure, figure being the collection of objects
	int findBottomBound();
	int findTopBound();
	int findRightBound();
	int findLeftBound();
...
public:
	...
	//GETTERS
	int getSize(); //Returns the size/dimension
	std::vector<T> getObjects();  //Returns the objectList

	void insertObject(T); //Insert a new object 
	void placeObjects(); //Builds the plate by placing objects from the objectList
